...
const createPatch = (opts: ConstructorParameters<typeof Patch>[0]) => new Patch(opts);
const createNodeAddV2 = (node: string, dot: unknown) => new NodeAdd(node, dot as ConstructorParameters<typeof NodeAdd>[1]);
 ...
 * @returns {any[]}
 */
function generateV5Patches(patchCount: number, options: { writerCount?: number; opsPerPatch?: number; includeRemoves?: boolean; seed?: number } = {}) {
  const {
    writerCount = 5,
    opsPerPatch = 3,
    includeRemoves = true,
  ...
  for (let i = 0; i < patchCount; i++) {
    ...
    patches.push({
      patch: createPatch({
        ...
        lamport,
        context,
        ops: ops as ConstructorParameters<typeof Patch>[0]['ops'],
      }),
      sha,
    });
  }

...
describe('WarpRuntime.watch() (PL/WATCH/1)', () => {
  ...
  describe('edge cases', () => {
    ...
    it('multiple watch handlers can coexist with different patterns', async () => {
      ...
      const userDiff = (userHandler.mock.calls[0] as any)[0];
      const orderDiff = (orderHandler.mock.calls[0] as any)[0];
    ...
    it('watch and subscribe can coexist', async () => {
      ...
      const watchDiff = (watchHandler.mock.calls[0] as any)[0];
      expect(watchDiff.nodes.added).toEqual(['user:alice']);

      // Subscribe sees everything
      const subscribeDiff = (subscribeHandler.mock.calls[0] as any)[0];

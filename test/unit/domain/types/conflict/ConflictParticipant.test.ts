...
describe('ConflictParticipant', () => {
  ...
  it('treats null causalRelationToWinner as undefined', () => {
    const p = new ConflictParticipant(({ ...VALID, causalRelationToWinner: null } as any));
    expect(p.causalRelationToWinner).toBeUndefined();
  });

  it('treats null notes as undefined', () => {
    const p = new ConflictParticipant(({ ...VALID, notes: null } as any));
    expect(p.notes).toBeUndefined();
  });

  it('rejects non-ConflictAnchor anchor', () => {
    expect(() => new ConflictParticipant(({ ...VALID, anchor: {} } as any))).toThrow('anchor must be a ConflictAnchor');
  });
  ...
  it('rejects non-boolean structurallyDistinctAlternative', () => {
    expect(() => new ConflictParticipant(({ ...VALID, structurallyDistinctAlternative: 1 } as any))).toThrow('must be a boolean');
  });

  it('rejects non-boolean replayableFromAnchors', () => {
    expect(() => new ConflictParticipant(({ ...VALID, replayableFromAnchors: 'yes' } as any))).toThrow('must be a boolean');
  });
});

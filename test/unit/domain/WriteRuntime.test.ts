...
describe('WriteRuntime admission classification', () => {
  ...
  it('rejects an invalid built sequence before publication begins', async () => {
    ...
    await expect(
      executeIntentWrite({
        ...
        intent: intent.node.add({ subject: 'entry:requested' }),
        commit: async (build) => {
          const capture = builder({ persistence, patchJournal: journal });
          capture.addNode('entry:unrequested');
          await build(capture);
          return await capture.commitWithEvidence();
        },
      }),

...
describe('PatchBuilderV2 content attachment', () => {
  ...
  describe('commit() with content blobs', () => {
    it('includes _blob/* entries in tree when content blobs exist', async () => {
      ...
      // writeTree should be called with both patch.cbor and _blob/0
      ...
      expect(treeEntries[1]).toBe(`100644 blob ${contentOid}\t_blob/0`);
    });
    ...
    it('includes multiple _blob/* entries for multiple attachments', async () => {
      let blobIdx = 0;
      ...
      expect(treeEntries[0]).toContain('patch.cbor');
      expect(treeEntries[1]).toBe(`100644 blob ${contentA}\t_blob/0`);
      expect(treeEntries[2]).toBe(`100644 blob ${contentB}\t_blob/1`);
    });
  });
});

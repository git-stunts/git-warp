...
describe('Canonical hash freeze', () => {
  ...
  it('all golden records pass verifyRecordId', async () => {
    for (const record of GOLDEN_CHAIN) {
      expect(await verifyRecordId(/** @type {any} */ (record))).toBe(true);
    }
  });

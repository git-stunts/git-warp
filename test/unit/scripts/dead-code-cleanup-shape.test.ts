...
describe('dead-code cleanup closeout', () => {
  ...
  it('makes the fake-model purge note own the live blocker explicitly', () => {
    const owningNote = readRepoFile('docs/method/graveyard/v17.0.0-residual-backlog/PROTO_purge-fake-models.md');
  ...
  it('removes the duplicate dead-code card from the live v17 lane', () => {
    expect(existsSync(`${repoRoot}docs/method/graveyard/v17.0.0-residual-backlog/SLUDGE_dead-code-cleanup.md`)).toBe(false);
  });
});

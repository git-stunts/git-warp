...
describe('incremental-index-updater closeout', () => {
  ...
  it('removes the stale live note and dead workload item', () => {
    ...
    expect(existsSync(`${repoRoot}docs/method/backlog/v17.0.0/GOD_incremental-index-updater.md`)).toBe(false);
  ...
  it('drops the dead god from downstream blocker lists', () => {
    ...
    const remainingBigFiles = readRepoFile('docs/method/backlog/v17.0.0/GOD_remaining-big-files.md');
    ...
    expect(remainingBigFiles).not.toContain('GOD_incremental-index-updater');

...
describe('content-access duplication closeout', () => {
  ...
  it('makes capability migration own the deferred content accessor surface', () => {
    const migrateNote = readRepoFile('docs/method/backlog/v17.0.0/API_migrate-consumers-to-capabilities.md');
  ...
  it('removes the stale live sludge note from the v17 lane', () => {
    expect(existsSync(`${repoRoot}docs/method/backlog/v17.0.0/SLUDGE_content-access-duplication.md`)).toBe(false);
  });
});

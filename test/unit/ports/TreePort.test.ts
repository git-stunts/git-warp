...
describe('TreePort', () => {
  ...
  it('concrete subclass satisfies the contract', async () => {
    class TestTree extends TreePort {
      async writeTree(_entries: string[]) { return 'tree-oid'; }
      async readTree(_treeOid: string) { return { 'file.txt': new Uint8Array([1]) }; }
      async readTreeOids(_treeOid: string) { return { 'file.txt': 'blob-oid' }; }

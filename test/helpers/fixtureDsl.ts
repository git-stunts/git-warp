...
export type GraphFixture = {
  ...
  props: Array<{ nodeId: string; key: string; value: unknown; lamport?: number }>;
  tombstones: { nodes?: Set<string>; edges?: Set<string> };
};
 ...
 */
export function toAdjacencyMaps(fixture: GraphFixture) {
 ...
 */
export function makeAdjacencyProvider(fixture: GraphFixture) {
...
type CrossProviderResult = { name: string; result: unknown; error: unknown };
...
export async function runCrossProvider({ providers, run, assert }: {
  ...
  providers: Array<{ name: string; provider: import('../../src/ports/NeighborProviderPort.ts').default }>;
  run: (engine: import('../../src/domain/services/query/GraphTraversal.ts').default) => Promise<unknown>;
  assert: (result: unknown) => void;
}) {
  const results: CrossProviderResult[] = [];
  ...
  for (const { name, result, error } of results) {
    if (error) {
      ...
      try {
        ...
        throw new Error(`Provider '${name}' threw unexpectedly: ${(error as Error).message}`);
      }
    } else {
      try {
        assert(result);
      } catch (e) {
        throw new Error(`Provider '${name}' failed assertion: ${(e as Error).message}`);
      }
    }
  }
}

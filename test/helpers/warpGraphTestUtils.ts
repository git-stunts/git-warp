 ...
 * const sha1 = getSha(1);
 */
export function createPopulatedMockPersistence(commits: Array<{ index: number; patch: unknown; parentIndex: number | null; writerId: string; lamport: number }>, graphName = 'test-graph'): { persistence: any; getSha: (index: number) => string } {
...
export function createMockPatchWithIO(
  { sha, graphName, writerId, lamport, ops, reads, writes, parentSha = null }: {
    sha: string; graphName: string; writerId: string; lamport: number;
    ops: unknown[]; reads?: unknown[]; writes?: unknown[];
  ...
  const patch: { schema: number; writer: string; lamport: number; context: Record<string, number>; ops: unknown[]; reads?: unknown[]; writes?: unknown[] } = {
    schema: 2,
    writer: writerId,
    lamport,
    context,
...
export function createMockPatch({
  ...
  sha: string; patchOid: string; graphName: string; writerId: string;
  lamport: number; ops?: unknown[]; parentSha?: string | null;
}) {
  const context = { [writerId]: lamport };
  const patch = {
    schema: 2,
 ...
 * })
 */
export function createPatch({ writer, lamport, ops, context }: { writer: string; lamport: number; ops: unknown[]; context?: unknown }): Patch {
  return new Patch({
    schema: 2,
    writer,
    lamport,
    ops: ops as Patch['ops'],
    context: (context || VersionVector.empty()) as Patch['context'],
  });
}

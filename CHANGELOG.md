...
- **Pre-push hook regression harness** — Added a focused Vitest source-level regression suite for `scripts/hooks/pre-push` that locks the “seven gates in parallel” header, quick-mode skip messaging, and all Gate 1–8 failure labels to the checked-in hook source.

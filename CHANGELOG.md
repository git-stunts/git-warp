...
- **`autoMaterialize` defaults to `true`**: Query methods now transparently call `materialize()` when no cached state exists. To preserve old behavior, pass `autoMaterialize: false` explicitly. See [migration guide](docs/topics/api-reference.md#migrating-from-automaterialize-false).

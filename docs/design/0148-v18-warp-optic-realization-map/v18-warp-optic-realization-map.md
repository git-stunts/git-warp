...
| `rho` | Set-side lowering surface that presents comparable claims over `chi` | `reducePatches`, `applyWithReceipt`, `JoinReducerSession`, op classes, `PatchDiff`, `syncDelta`, materialize-coordinate paths | Generated lowering inputs for receipt, settlement, and runtime-boundary families |

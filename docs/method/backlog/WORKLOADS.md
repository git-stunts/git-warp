...
| `WL-38-v17-shadow-trie-materialization-core` | 9 | `PROTO_orsetlike-contract`, `PROTO_shadow-trie-orset`, `PROTO_trie-compaction`, `PROTO_state-session-async`, `PROTO_gc-state-session`, `PROTO_materialize-integration`, `PROTO_index-builder-trie-iteration`, `PERF_trie-geometry-and-memory-profile`, `TRUST_shadow-trie-semilattice-pbt` | Remaining shadow-trie engine, async firewall, and kernel integration trunk after the checkpoint/snapshot unification cycle landed |

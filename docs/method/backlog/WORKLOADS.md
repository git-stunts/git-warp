...
| `WL-38-v17-shadow-trie-materialization-core` | 8 | `PROTO_orsetlike-contract`, `PROTO_shadow-trie-orset`, `PROTO_trie-compaction`, `PROTO_state-session-async`, `PROTO_gc-state-session`, `PROTO_index-builder-trie-iteration`, `PERF_trie-geometry-and-memory-profile`, `TRUST_shadow-trie-semilattice-pbt` | Remaining shadow-trie engine, async firewall, and post-materialization kernel follow-through after the checkpoint/snapshot unification cycle landed |

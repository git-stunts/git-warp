...
| `WL-38-v17-shadow-trie-materialization-core` | 11 | `PROTO_orsetlike-contract`, `PROTO_shadow-trie-orset`, `PROTO_trie-compaction`, `PROTO_state-session-async`, `PROTO_gc-state-session`, `PROTO_joinreducer-state-session`, `PROTO_checkpoint-envelope-publication`, `PROTO_materialize-integration`, `PROTO_index-builder-trie-iteration`, `PERF_trie-geometry-and-memory-profile`, `TRUST_shadow-trie-semilattice-pbt` | Remaining shadow-trie engine, async firewall, and kernel integration trunk after consumed foundation cycles were removed from backlog |

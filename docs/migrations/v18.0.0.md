...
| Optic reads | `graph.query.worldline().optic()` | `worldline.prepareOpticBasis()`, then `worldline.coordinate().optic()` |
...
`E_OPTIC_NO_BOUNDED_BASIS`, prepare the optic basis before capturing the
coordinate, repair the checkpoint-tail evidence, or use `events.live()`,
`events.seek(...)`, or an observer read when you do not need Optic identity.

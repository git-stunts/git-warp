...
import { loadCheckpoint, create as createCheckpointCommit, isV5CheckpointSchema } from './CheckpointService.js';
import { decodePatchMessage, detectMessageKind, encodeAnchorMessage } from './WarpMessageCodec.js';
import { shouldRunGC, executeGC } from './GCPolicy.js';
import { collectGCMetrics } from './GCMetrics.js';
import { computeAppliedVV } from './CheckpointSerializerV5.js';
...
export default class CheckpointController {
  ...
  async createCheckpoint() {
    ...
    try {
      ...
      let state;
      try {
        state = /** @type {import('./JoinReducer.js').WarpStateV5} */ ((h._cachedState && !h._stateDirty)
          ? h._cachedState
          : await h.materialize());
      } finally {
   ...
   *
   * @returns {Promise<{state: import('./JoinReducer.js').WarpStateV5, frontier: Map<string, string>, stateHash: string, schema: number, provenanceIndex?: import('./ProvenanceIndex.js').ProvenanceIndex, indexShardOids?: Record<string, string>|null}|null>}
   */

...
| `src/domain/orset/trie/` | `TrieStorePort.ts`, `TrieBranchEntries.ts` (cycle 0026); `GitTrieStoreAdapter.ts` lives under `src/infrastructure/adapters/` (cycle 0028); planned: `TrieLeaf.ts`, `TrieBranch.ts`, `TrieCursor.ts`, `TrieGeometry.ts`, `PageCache.ts`, `TrieFlusher.ts` | `PROTO_git-trie-store-port`, `INFRA_git-trie-store-adapter`, `PROTO_trie-codec-and-geometry`, `PROTO_trie-cursor`, `PERF_lru-page-cache`, `PROTO_trie-flush` | port: cycle 0026; adapter: cycle 0028; rest pending |

...
| `src/domain/orset/trie/` | `TrieStorePort.ts`, `TrieBranchEntries.ts` (cycle 0026); `TrieGeometry.ts`, `TrieLeaf.ts`, `TrieBranch.ts` (cycle 0027); planned: `TrieCursor.ts`, `PageCache.ts`, `TrieFlusher.ts` | `PROTO_git-trie-store-port`, `PROTO_trie-codec-and-geometry`, `PROTO_trie-cursor`, `PERF_lru-page-cache`, `PROTO_trie-flush` | port: cycle 0026; codec+geometry: cycle 0027; rest pending |

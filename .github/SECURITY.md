...
| `roaring-wasm` | Runtime | Low-Medium | WebAssembly build of CRoaring, loaded lazily via `await import()` in `RoaringLoaderAdapter`. Memory-safety faults stay inside the module's linear memory: they can yield a wrong bitmap result or a trap, but cannot reach host process memory. No network-facing input |
...
|---|---|---|---|---|
| `roaring-wasm` bitmap faults could corrupt in-module state | Low | @flyingrobots | 2026-08-01 | WASM linear-memory sandbox prevents host memory access; failure mode is a wrong result or a trap, not host corruption; no untrusted input reaches bitmap code directly |
| `cbor-x` parser handles untrusted sync payloads | Medium | @flyingrobots | 2026-08-01 | 4 MB body size limit in HttpSyncServer; schema validation post-parse |
| Nonce cache lost on restart allows replay within clock skew window | Low | @flyingrobots | 2026-08-01 | 5-minute TTL; recommend TLS in production |
| No rate limiting on sync endpoint | Low | @flyingrobots | 2026-08-01 | Deploy behind reverse proxy with rate limiting |

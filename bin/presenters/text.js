 ...
 * @typedef {{ graph?: string, action: string, tick?: number, maxTick?: number, ticks?: number[], nodes?: number, edges?: number, patchCount?: number, perWriter?: Record<string, unknown>, diff?: { nodes?: number, edges?: number } | null, tickReceipt?: Record<string, unknown>, structuralDiff?: import('../../src/domain/services/state/StateDiff.js').StateDiffResult | null, cursor?: { active: boolean, tick?: number }, message?: string, cursors?: Array<{ name: string, tick: number }>, activeTick?: number | null, name?: string, diffBaseline?: string, baselineTick?: number | null, truncated?: boolean, totalChanges?: number, shownChanges?: number }} SeekPayload
...
function renderStrandDebugContextLines(strand) {
  ...
  return [
    `Strand Overlay: head=${strand.overlayHeadPatchSha ?? 'none'} patches=${strand.overlayPatchCount} base-ceiling=${strand.baseLamportCeiling ?? 'latest'} writable=${strand.overlayWritable ? 'yes' : 'no'}`,
    `Strand Braids: ${braidedStrandIds.length === 0 ? 'none' : braidedStrandIds.join(', ')}`,
  ];
}
...
export function renderStrand(payload) {
  ...
  const lines = [
    `Graph: ${payload.graph}`,
    `Strand Action: ${payload.strandAction}`,
    ...renderStrandDescriptorLines(payload.strand),
  ];
  return `${lines.join('\n')}\n`;
}

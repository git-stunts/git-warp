 ...
 * @returns {string | null}
 */
export function parseFenceLanguage(info: string): string | null {
 ...
 */
function createMarkdownCodeSampleIssue(filePath: string, line: number, language: string, message: string) {
  return {
    filePath,
    line,
    column: 1,
 ...
 */
function extractMarkdownCodeSamplesWithIssues(markdown: string, filePath: string) {
  const lines = markdown.split('\n');
  const samples: { filePath: string, language: string, code: string, fenceLine: number, startLine: number }[] = [];
  const issues: { filePath: string, line: number, column: number, message: string, language: string }[] = [];
  let activeFence: { marker: string, markerLength: number, language: string | null, fenceLine: number, codeLines: string[] } | null = null;
 ...
 */
export function extractMarkdownCodeSamples(markdown: string, filePath: string) {
 ...
 * @returns {string[]}
 */
export function collectMarkdownFiles(startPath: string = root): string[] {
 ...
 * @returns {MarkdownCodeSampleIssue[]}
 */
export function lintMarkdownCodeSample(sample: { filePath: string, language: string, code: string, fenceLine: number, startLine: number }) {
  ...
  const diagnostics: ReadonlyArray<ts.DiagnosticWithLocation> =
    (sourceFile as ts.SourceFile & { parseDiagnostics?: ReadonlyArray<ts.DiagnosticWithLocation> })
      .parseDiagnostics || [];

  return diagnostics.map((diagnostic: ts.DiagnosticWithLocation) => {
 ...
 * @returns {MarkdownCodeSampleIssue[]}
 */
export function lintMarkdownCodeSamples(markdownFiles: string[]) {
  const issues: { filePath: string, line: number, column: number, message: string, language: string }[] = [];
